...
def inputs(n_coll, device):
    ...
    t0 = torch.zeros((50, 1), device=device)
    x0 = -1 + 2*torch.rand((50, 1), device=device)
    U1 = -torch.sin(torch.pi * x0)
    T1 = torch.cat((t0, x0), dim=1)

    t1 = torch.rand((25, 1), device=device)
    x1 = -torch.ones((25, 1), device=device)
    U2 = torch.zeros((25, 1), device=device)
    T2 = torch.cat((t1, x1), dim=1)

    t2 = torch.rand((25, 1), device=device)
    x2 = torch.ones((25, 1), device=device)
    U3 = torch.zeros((25, 1), device=device)
...
def train(model, optimizer, epochs, n_coll, device):
    ...
    Xbc, Ubc, t_func, x_func = inputs(n_coll, device)
